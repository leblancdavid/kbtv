    ...
    public partial class EvidenceModal : Control
    {
        ...
        private void SetupModal()
        {
            ...
            if (_caller != null)
            {
                ...
                // Caller name label
                _callerNameLabel = new Label
                {
                    Text = $"Caller: {_caller.Name}",
                    HorizontalAlignment = HorizontalAlignment.Center
                };
                _callerNameLabel.AddThemeFontSizeOverride("font_size", 14);
                _callerNameLabel.AddThemeColorOverride("font_color", UIColors.TEXT_SECONDARY);
                topContainer.AddChild(_callerNameLabel);
                
                // Patience progress bar (gray)
                _patienceProgressBar = new ProgressBar
                {
                    MinValue = 0,
                    MaxValue = _initialPatience > 0 ? _initialPatience : 100,
                    Value = _initialPatience,
                    CustomMinimumSize = new Vector2(200, 20)
                };
                
                // Style background (dark gray for empty portion)
                ...
                _patienceProgressBar.AddThemeStyleboxOverride("background", bgStyle);
                
                // Style fill (medium gray for filled portion)
                ...
                _patienceProgressBar.AddThemeStyleboxOverride("fill", progressStyle);
                
                topContainer.AddChild(_patienceProgressBar);
                
                // Add to modal panel at top
                var modalPanel = GetNodeOrNull<Control>("ModalPanel");
                if (modalPanel != null && modalPanel.GetChildCount() > 0)
                {
        ...
        private void ShowSuccessMessage()
        {
            ...
            // Add to ScrollContainer content - FIXED node path
            var guessHistoryContent = _guessHistory.GetChild(0) as VBoxContainer;
            if (guessHistoryContent != null)
            {
        ...
        {
            _titleLabel ??= GetNodeOrNull<Label>("ModalPanel/TitleLabel");
            _wordDisplay ??= GetNodeOrNull<Control>("ModalPanel/VBoxContainer/WordDisplay");
            _alphabetDisplay ??= GetNodeOrNull<Control>("ModalPanel/MainHBoxContainer/RightPanel/AlphabetDisplay");
            _attemptsLabel ??= GetNodeOrNull<Label>("ModalPanel/MainHBoxContainer/LeftPanel/AttemptsLabel");
            _collectButton ??= GetNodeOrNull<Button>("ModalPanel/CollectButton");
            _guessHistory ??= GetNodeOrNull<Control>("ModalPanel/MainHBoxContainer/LeftPanel/GuessHistory");
        }
